...
Debianは、Debian Projectによって開発されているフリーなOSである。UbuntuやRaspberry Pi OSなどの多くのソフトウェアの派生元となるOSとなっている。校舎のPCにインストールされているUbuntu 22.04 LTSも、Debianをベースにして開発された。安定性がある一方でリリースが遅く、新機能などを利用することができないというデメリットがある。
...
どちらも、UNIXのシステムではDAC (任意アクセス制御)が採用されている中で、MAC(強制アクセス制御)を可能にするものである。つまり、sudoなどの権限を通して強制的に実行することができなくなるようにするもので、root以外が権限設定を変更することができなくなるようにするものである。
...
どちらもハイパーバイザ、ホストのリソースとゲストのリソースをつなぐソフトウェアである。
...
公式の方法に従って、インストールメディアを入手しインストールした。その上で、VirtualBox上で起動ディスクとしてISOファイルを指定して起動した。
...
1. VirtualBoxから新規仮想マシンを作成、メモリやCPUコア数の割当をし、インストールメディアを初期状態としてインストールする。
...
- hibernationは使用しないため
- もともと2GBを想定していたが、サーバーというクラッシュが致命的になりやすい環境ではゆとりを持つことがより良いと感じたから
